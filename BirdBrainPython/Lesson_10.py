...
def drawCircle(): #make the finch go in a circle
    for i in range(180):
        bird.setMove('F', 2, 100)
        bird.setTurn('R', 2, 100)

def selfNav(): #navigate around without hitting any obstacles. If an obstacle is seen, readjust
    for i in range(10):
...
def drawSquare2(): #draw 5 squares of random size in length
...
def blinkAll(red, green, blue): #while button A isn't pressed, blink all the lights in the color determined by the user
...
def isLevel(): #if the finch isn't level, then print false and blink all lights. If finch is level, then print true.
